...
const messages = [
    ...
    "Mong em luôn nở 1 nụ cười thật tươi!",
];
const fallingTexts = [];
...
function createBackgroundStars() {
    for (let i = 0; i < 200; i++) {
        stars.push({
            ...
            brightness: Math.random() * 0.1 + 0.2
        });
    }
}
...
setInterval(() => {
    if (Math.random() < 0.08) createFallingText();
}, 1000);
...
const fallingImages = []; // Mảng lưu trữ các ảnh đang rơi

window.addEventListener('resize', () => {
    width = canvas.width = window.innerWidth;
    height = canvas.height = window.innerHeight;
    heartScale = Math.min(width, height) * 0.03;
    heartStars.length = 0;
    stars.length = 0;
    createHeartStars();
...
setInterval(() => {
    if (Math.random() < 0.15) createMeteor();   ``
}, 1000);
